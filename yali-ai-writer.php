...
// 编辑器助手组件将在 content_auto_manager_init 函数中统一加载
...
// 加载 Composer 自动加载器
if (file_exists(CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'vendor/autoload.php')) {
    require_once CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'vendor/autoload.php';
}
...
add_action('wp_ajax_content_auto_delete_article_task', 'content_auto_delete_article_task');
...
add_action('wp_ajax_content_auto_fetch_pollinations_models', 'content_auto_fetch_pollinations_models_handler');
add_action('wp_ajax_content_auto_get_pollinations_account_info', 'content_auto_manager_get_pollinations_account_info');
...
function content_auto_manager_uninstall() {
    ...
    delete_option('content_auto_manager_allowed_categories');  // 删除分类过滤设置
    delete_option('content_auto_manager_category_filter_enabled');  // 删除分类过滤启用状态
    delete_option('content_auto_manager_license_key');  // 删除授权密钥
    delete_option('content_auto_manager_license_data');  // 删除授权数据
...
function content_auto_manager_init() {
    ...
    if (is_admin()) {
        ...
        ContentAuto_PublishRulesAdminPage::init();
    }
    ...
    // 6. 初始化编辑器助手
    if (file_exists(CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'editor-assistant/class-prompt-manager.php')) {
        require_once CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'editor-assistant/class-prompt-manager.php';
    }
    if (file_exists(CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'editor-assistant/class-asset-loader.php')) {
        require_once CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'editor-assistant/class-asset-loader.php';
        new ContentAuto_Editor_Asset_Loader();
    }
    if (file_exists(CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'editor-assistant/class-database-migration.php')) {
        require_once CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'editor-assistant/class-database-migration.php';
        new ContentAuto_Editor_Database_Migration();
    }
    if (file_exists(CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'editor-assistant/gutenberg/index.php')) {
        require_once CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'editor-assistant/gutenberg/index.php';
    }
}
...
function content_auto_manager_add_post_columns($columns) {
    $columns['content_auto_manager'] = '自动生成';
    return $columns;
}
...
// 加载依赖管理器 (CommonMark)
require_once CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'gsc-auth/init.php';
require_once CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'admin/class-dependency-manager.php';
new Content_Auto_Dependency_Manager();
