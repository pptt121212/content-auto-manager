...
class Yali_AI_Writer_ArticleStructureAdminPage {
    ...
    public function ajax_get_content_angles() {
        ...
        if (is_wp_error($db_angles)) {
            wp_send_json_error(['message' => __('获取内容角度列表失败: ', 'yali-ai-writer') . $db_angles->get_error_message()]);
            return;
        }
        ...
        wp_send_json_success([
            ...
            'dynamic_angles' => array_values($dynamic_angles),
            'fixed_angles_localized' => $fixed_angles_with_display,
            'dynamic_angles_localized' => $dynamic_angles_with_display,
        ]);
    }
    ...
    public function ajax_get_article_structures() {
        ...
        if ($wpdb->last_error) {
            wp_send_json_error(['message' => __('数据库查询失败: ', 'yali-ai-writer') . $wpdb->last_error]);
            return;
        }
    ...
    public function ajax_generate_article_structures() {
        ...
        if (!isset($_POST['angle']) || empty($_POST['angle'])) {
            wp_send_json_error(['message' => __('未提供内容角度。', 'yali-ai-writer')]);
            return;
        }
        ...
        if (!file_exists($prompt_template_path)) {
            wp_send_json_error(['message' => __('文章结构生成提示词模板文件不存在。', 'yali-ai-writer')]);
            return;
        }
        ...
        for ($i = 0; $i < $num_to_generate; $i++) {
            ...
            $raw_response = $api_handler->generate_content($prompt, 'structure_generation', [
                'return_usage' => true // 增加 return_usage 以便获取 finish_reason
            ]);

            // 提取内容（return_usage 使响应格式变为数组）
            $content_to_parse = is_array($raw_response) && isset($raw_response['content']) ? $raw_response['content'] : $raw_response;

            // 增强的响应解析逻辑
            $structure_data = $this->robust_parse_api_response($content_to_parse, $logger, $angle, $i + 1);
            ...
            if ($structure_data !== false) {
                ...
                if ($json_error === JSON_ERROR_NONE && isset($structure_data['title']) && isset($structure_data['structure'])) {
                    ...
                    if ($new_id) {
                        ...
                        $logger->error("Structure generation loop: DB insert failed for angle {$angle}", [
                            ...
                            'json_string' => $content_to_parse, // 这里可能未定义 json_string 原代码逻辑中有，我先给它换成 content_to_parse
                            'attempt' => $i + 1,
                            'progress' => "({$successful_creations}/{$num_to_generate})"
                        ]);
                    }
                ...
                $logger->error("Structure generation loop: API响应解析完全失败", [
                    'angle' => $angle,
                    'raw_response_type' => gettype($content_to_parse),
                    'raw_response_preview' => is_string($content_to_parse) ? substr($content_to_parse, 0, 200) : $content_to_parse,
                    'attempt' => $i + 1
                ]);
            }
        ...
        if (empty($newly_created)) {
            wp_send_json_error(['message' => __('生成失败，请检查API配置或稍后重试', 'yali-ai-writer')]);
            return;
        }
        ...
        if ($structures_created > 0) {
            $message = $vectors_generated > 0 ? 
                __('✅ 成功生成1个新结构并生成了向量', 'yali-ai-writer') : 
                __('✅ 成功生成1个新结构', 'yali-ai-writer');
            wp_send_json_success(['message' => $message]);
        } else {
            wp_send_json_error(['message' => __('生成失败，请检查API配置', 'yali-ai-writer')]);
        }
    }
    ...
    public function ajax_delete_article_structure() {
        ...
        try {
            ...
            if ($result === false) {
                throw new Exception(__('删除文章结构失败: ', 'yali-ai-writer') . $wpdb->last_error);
            } else if ($result === 0) {
                throw new Exception(__('要删除的结构不存在', 'yali-ai-writer'));
            }
            ...
            $message = __('文章结构已删除', 'yali-ai-writer');
            if ($cleaned_meta_count > 0) {
                $message .= sprintf(__('，同时清理了 %d 个文章关联', 'yali-ai-writer'), $cleaned_meta_count);
            }
    ...
    public function ajax_delete_dynamic_angle() {
        ...
        if (!isset($_POST['angle']) || empty($_POST['angle'])) {
            wp_send_json_error(['message' => __('未提供角度名称', 'yali-ai-writer')]);
            return;
        }
        ...
        if (in_array($angle, $fixed_angles)) {
            wp_send_json_error(['message' => __('不能删除固定的内容角度', 'yali-ai-writer')]);
            return;
        }
        ...
        try {
            ...
            wp_send_json_success([
                'message' => sprintf(
                    __('成功删除动态角度"%s"：%d个主题已重新分配到固定角度，%d个相关结构已删除，%d个文章关联已清理', 'yali-ai-writer'),
                    $angle,
                    $updated_topics,
                    $deleted_structures,
                    $total_cleaned_meta
            ...
            // 回滚事务
            $wpdb->query('ROLLBACK');
            wp_send_json_error(['message' => __('删除失败: ', 'yali-ai-writer') . $e->getMessage()]);
        }
    }
