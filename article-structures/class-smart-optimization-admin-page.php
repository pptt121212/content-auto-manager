...
class ContentAuto_SmartOptimizationAdminPage {
    ...
    public function render_page() {
        // 读取视图、CSS 和 JS 文件
        $view_html = file_get_contents(CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'article-structures/views/smart-optimization-settings.php');
        $style_css = file_get_contents(CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'article-structures/assets/css/smart-optimization-settings.css');
        $script_js = file_get_contents(CONTENT_AUTO_MANAGER_PLUGIN_DIR . 'article-structures/assets/js/smart-optimization-settings.js');
        ...
        <div class="wrap">
            ...
            <script type="text/javascript">
                const smartOptimization = <?php echo json_encode($localized_data); ?>;
            </script>
            <script type="text/javascript">
                <?php echo $script_js; ?>
            </script>
    ...
    public function ajax_save_config() {
        ...
        if ($result) {
            wp_send_json_success(array('message' => '配置已保存'));
        } else {
    ...
    public function ajax_save_configs() {
        ...
        if (!isset($_POST['configs']) || !is_array($_POST['configs'])) {
            wp_send_json_error(array('message' => '缺少配置数据'));
            return;
        }
    ...
    public function ajax_reset_configs() {
        ...
        if ($result) {
            wp_send_json_success(array('message' => '配置已恢复为默认值'));
        } else {
    ...
    public function ajax_get_pending_analysis_count() {
        ...
        try {
            ...
            wp_send_json_success(array(
                ...
                'message' => $total > 0 ? "共有 {$total} 篇待处理文章" : '没有待处理的高表现文章'
            ));
            
        } catch (Exception $e) {
            wp_send_json_error(array('message' => '获取数量失败：' . $e->getMessage()));
        }
    }
    ...
    public function ajax_process_single_article() {
        ...
        if (!isset($_POST['post_id']) || !is_numeric($_POST['post_id'])) {
            wp_send_json_error(array('message' => '缺少文章ID'));
            return;
        }
        ...
        try {
            ...
            if ($result['success']) {
                ...
                if (isset($result['skipped']) && $result['skipped']) {
                    wp_send_json_success(array(
                        'message' => '分析已跳过：' . ($result['reason'] ?? '未知原因'),
                        'structures_created' => 0
                    ));
                } else if ($result['structures_created'] > 0) {
                    wp_send_json_success(array(
                        'message' => '处理了 1 篇文章，创建了 1 个新结构。',
    ...
    public function ajax_update_popularity_indices() {
        ...
        wp_send_json_success(array(
            'message' => '已更新 ' . $updated_count . ' 个结构的受欢迎度指数'
        ));
    }
...
function render_structure_analysis_metabox($post) {
    ...
    <div id="structure-analysis-container">
        <?php if ($structure_id): ?>
            <p><strong>当前关联结构ID:</strong> <?php echo esc_html($structure_id); ?></p>
        <?php endif; ?>
        
        <p>从此文章提取结构特征，生成可复用的文章结构模板。</p>
    ...
    jQuery(document).ready(function($) {
        $('#analyze-structure-btn').on('click', function() {
            ...
            $.ajax({
                ...
                error: function() {
                    $result.html('<div class="notice notice-error inline"><p>请求失败，请重试</p></div>');
                },
                complete: function() {
