...
class ContentAuto_TopicTaskManager {
    ...
    private function build_reference_material_block($content, $rule_id) {
        ...
        // 判断规则类型：需要参考资料的规则类型
        $needs_reference_material = false;
        
        if (isset($first_item['keyword']) && !empty($first_item['keyword'])) {
            // 导入关键词规则
            $needs_reference_material = true;
        } elseif (isset($first_item['category_name']) && !empty($first_item['category_name'])) {
            // 随机分类规则
            $needs_reference_material = true;
        }
        
        // 如果不需要参考资料，返回空字符串
        if (!$needs_reference_material) {
            return '';
        }
