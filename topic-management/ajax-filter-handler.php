...
add_action('wp_ajax_cam_delete_topics', 'cam_ajax_bulk_delete_topics'); // Alias to handle both names
...
function cam_ajax_detect_duplicates() {
    ...
    try {
        ...
        wp_send_json_error(array('message' => __('检测重复标题时出错: ', 'yali-ai-writer') . $e->getMessage()));
    }
}
...
function cam_ajax_bulk_delete_topics() {
    ...
    // 获取ID列表，兼容 topic_ids 和 ids
    $id_raw = isset($_POST['topic_ids']) ? $_POST['topic_ids'] : (isset($_POST['ids']) ? $_POST['ids'] : array());
    $topic_ids = array_map('intval', (array)$id_raw);
    
    if (empty($topic_ids)) {
        wp_send_json_error(array('message' => __('请选择要删除的主题', 'yali-ai-writer')));
    }
...
function cam_ajax_delete_duplicate_topics() {
    ...
    if (!in_array($duplicate_type, array('exact', 'similar', 'all'))) {
        wp_send_json_error(array('message' => __('无效的重复类型', 'yali-ai-writer')));
    }
...
function cam_ajax_delete_all_filtered_topics() {
    ...
    if ($filters['status'] !== 'unused') {
        wp_send_json_error(array('message' => __('只能批量删除"未使用"状态的主题', 'yali-ai-writer')));
    }
...
function cam_ajax_bulk_generate_references() {
    ...
    if (empty($topic_ids)) {
        wp_send_json_error(array('message' => __('请选择要生成参考资料的主题', 'yali-ai-writer')));
    }
    ...
    $message_parts = array();
    if ($queued_count > 0) {
        $message_parts[] = sprintf(__('成功加入队列 %d 个主题', 'yali-ai-writer'), $queued_count);
    }
    if ($skipped_existing > 0) {
        $message_parts[] = sprintf(__('跳过 %d 个已有参考资料的主题', 'yali-ai-writer'), $skipped_existing);
    }
    if ($skipped_in_progress > 0) {
        $message_parts[] = sprintf(__('跳过 %d 个正在处理中的主题', 'yali-ai-writer'), $skipped_in_progress);
    }
    
    if (empty($message_parts)) {
        wp_send_json_error(array('message' => __('没有符合条件的主题可以处理', 'yali-ai-writer')));
    }
    
    $message = sprintf(__('操作完成：%s。', 'yali-ai-writer'), implode(__('，', 'yali-ai-writer'), $message_parts));
