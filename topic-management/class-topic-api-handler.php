...
class ContentAuto_TopicApiHandler {
    ...
    public function generate_topics($prompt, $count, $rule_id = null, $rule_item_index = null) {
        ...
        for ($attempt = 0; $attempt <= $max_retries; $attempt++) {
            ...
            $result_text = $unified_handler->generate_content($full_prompt, 'topic_generation', [
                'rule_id' => $rule_id,
            ...
            if (!is_array($result_text) || !isset($result_text['error'])) {
                // 4. 【追溯业务逻辑：解析识别】
                $topics = $this->parse_api_content($result_text, $count, $rule_id, $rule_item_index);
                ...
                $last_error = array(
                    'stage' => '内容解析', 
                    'message' => (isset($topics['error']) ? $topics['error'] : '无法从API返回的内容中提取有效主题')
                );
            } else {
