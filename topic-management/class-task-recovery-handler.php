...
class ContentAuto_TaskRecoveryHandler {
    ...
    public function recover_inconsistent_task_state($task_id, $task_type = 'topic_task') {
        ...
        if ($task['status'] === 'processing') {
            ...
            if (empty($processing_queue)) {
                $recovery_actions[] = __("将任务状态从processing改为pending", 'yali-ai-writer');
        ...
        if (!empty($completed_queue)) {
            ...
            if ($task['current_processing_item'] < $completed_count) {
                ...
                $recovery_actions[] = sprintf(__("更新current_processing_item从%1$d到%2$d", 'yali-ai-writer'), $task['current_processing_item'], $new_processing_item);
                $wpdb->update($task_table, array(
        ...
        if (!empty($failed_queue) && $task['status'] !== 'failed') {
            ...
            if ($task_type === 'article') {
                ...
                if ($total_processed >= $task['total_topics']) {
                    $recovery_actions[] = __("所有文章子任务已完成，设置任务状态为failed", 'yali-ai-writer');
                    $failed_error = __("文章任务包含失败的子项目", 'yali-ai-writer');
                    $wpdb->update($task_table, array(
                        'status' => 'failed',
                ...
                $recovery_actions[] = __("将任务状态设置为failed，因为有失败的队列项", 'yali-ai-writer');
                $failed_error = __("任务包含失败的子项目", 'yali-ai-writer');
                $wpdb->update($task_table, array(
                    'status' => 'failed',
                    'error_message' => $failed_error,
                    'updated_at' => current_time('mysql')
        ...
        if (empty($pending_queue) && $task['status'] === 'pending' && $task['current_processing_item'] < $task['total_rule_items']) {
            ...
            if ($task_type === 'article') {
                ...
                $recovery_actions[] = sprintf(__("重新创建队列项: subtask_id=%1$s, task_type=%2$s", 'yali-ai-writer'), $next_subtask_id, $task_type);
        ...
        if ($inconsistencies_found && $this->logger && !empty($recovery_actions)) {
            $this->logger->log_success('TASK_RECOVERY', __('任务状态恢复完成: ', 'yali-ai-writer') . implode('; ', $recovery_actions), ['task_id' => $task_id]);
        }
        ...
        foreach ($hanging_queue_tasks as $task) {
            ...
            if (!$topic_exists) {
                ...
                $wpdb->update($queue_table, [
                    'status' => 'failed',
                    'error_message' => __('恢复失败：主题已被删除', 'yali-ai-writer'),
                    'updated_at' => current_time('mysql')
                ], ['id' => $task['id']]);
                $failed_count++;
                continue;
            ...
            if ($current_retry >= $max_retries) {
                ...
                $wpdb->update($topics_table, [
                    ...
                    'material_search_error' => __('任务超时，已达最大重试次数', 'yali-ai-writer')
                ], ['id' => $topic_id]);
    ...
    private function recover_hanging_task($task_id, $task_type = 'topic_task') {
        ...
        $recovery_actions[] = __("重置任务状态为pending", 'yali-ai-writer');
        ...
        foreach ($processing_queue as $queue_item) {
            ...
            $wpdb->update($queue_table, array(
                'status' => 'pending',
                'updated_at' => current_time('mysql'),
                'error_message' => __('自动恢复: 重置挂起的处理状态', 'yali-ai-writer')
            ), array('subtask_id' => $queue_item['subtask_id']));
            $recovery_actions[] = sprintf(__("重置队列项%s为pending", 'yali-ai-writer'), $queue_item['subtask_id']);
        }
        
        // 记录恢复操作（简化版本，不写入历史表）
        if ($this->logger && !empty($recovery_actions)) {
            $this->logger->log_success('HANGING_TASK_RECOVERY', __('挂起任务自动恢复完成: ', 'yali-ai-writer') . implode('; ', $recovery_actions), ['task_id' => $task_id]);
        }
    ...
    public function smart_retry_task($task_id, $task_type = 'topic_task') {
        ...
        switch ($retry_strategy) {
            ...
            case 'manual':
                // 需要手动干预 - 严重错误
                if ($this->logger) {
                    $this->logger->log_warning('MANUAL_RETRY_REQUIRED', __('任务需要手动干预，错误类型: ', 'yali-ai-writer') . $error_message, ['task_id' => $task_id]);
                }
                return false;
    ...
    private function retry_entire_task($task_id, $task_type = 'topic_task', $force_retry = false) {
        ...
        if ($this->logger) {
            $this->logger->log_success('RETRY_ENTIRE_TASK_START', __("开始重试整个任务。", 'yali-ai-writer'), ['task_id' => $task_id, 'force_retry' => $force_retry]);
        }
        ...
        if (empty($failed_subtasks)) {
            if ($this->logger) {
                $this->logger->log_warning('RETRY_NO_FAILED_SUBTASKS', __("任务重试请求，但未找到失败的子任务。将仅重置主任务状态。", 'yali-ai-writer'), ['task_id' => $task_id]);
            }
            // 没有失败的子任务，无需重试
        ...
        if ($this->logger) {
            $this->logger->log_success('RETRY_FOUND_FAILED_SUBTASKS', sprintf(__("找到 %d 个失败的子任务准备重试。", 'yali-ai-writer'), count($failed_subtasks)), ['task_id' => $task_id, 'subtask_ids' => wp_list_pluck($failed_subtasks, 'subtask_id')]);
        }
        ...
        foreach ($failed_subtasks as $subtask) {
            ...
            if (!$force_retry && $subtask->retry_count >= $max_retries) {
                if ($this->logger) {
                    $this->logger->log_warning('RETRY_LIMIT_EXCEEDED', 
                        sprintf(__("子任务%1$s已达到最大重试次数(%2$d)，跳过重试", 'yali-ai-writer'), $subtask->subtask_id, $max_retries), 
            ...
            if ($this->logger) {
                $this->logger->log_success('SUBTASK_UPDATE_RESULT', 
                    sprintf(__("子任务更新结果: subtask_id=%1\$s, new_retry_count=%2\$d, delay=%3\$ds", 'yali-ai-writer'), $subtask->subtask_id, $new_retry_count, $retry_delay), 
                    ['task_id' => $task_id]
                );
            }
        }
        ...
        if ($this->logger) {
            $this->logger->log_success('RETRY_SUBTASKS_UPDATED', sprintf(__("成功更新了 %d 个子任务的状态为 'pending'。", 'yali-ai-writer'), $updated_count), ['task_id' => $task_id]);
        }
        ...
        if ($updated_count > 0) {
            ...
            if ($this->logger) {
                $this->logger->log_success('TASK_RETRY_INITIATED', sprintf(__("任务重试已启动，%d个失败的子任务被重新排队。", 'yali-ai-writer'), $updated_count), ['task_id' => $task_id]);
            }
            return true;
        }

        if ($this->logger) {
            $this->logger->log_error('RETRY_UPDATE_FAILED', __("尝试重置子任务状态，但数据库未报告任何行被更新。", 'yali-ai-writer'), ['task_id' => $task_id]);
        }
        return false;
    }
    ...
    private function detect_article_specific_inconsistencies($task, $queue_items, &$recovery_actions) {
        ...
        if (!empty($missing_reference_items)) {
            foreach ($missing_reference_items as $item) {
                ...
                if ($available_topic_id) {
                    ...
                    $recovery_actions[] = sprintf(__("为队列项%1$s分配reference_id: %2$s", 'yali-ai-writer'), $item['subtask_id'], $available_topic_id);
                    $inconsistencies_found = true;
                }
            }
        }
        ...
        foreach ($queue_items as $item) {
            if (!empty($item['reference_id'])) {
                ...
                if ($subtask_recorded_status !== $expected_subtask_status) {
                    ...
                    $recovery_actions[] = sprintf(__("同步主题%1$s的subtask状态: %2$s -> %3$s", 'yali-ai-writer'), $topic_id, $subtask_recorded_status, $expected_subtask_status);
                }
            }
        }
        ...
        if ($task['completed_topics'] !== $completed_count || $task['failed_topics'] !== $failed_count) {
            ...
            $recovery_actions[] = sprintf(__("修正计数: completed_topics=%1$d, failed_topics=%2$d", 'yali-ai-writer'), $completed_count, $failed_count);
            $inconsistencies_found = true;
        }
    ...
    private function sync_article_task_progress($task, $completed_queue, &$recovery_actions) {
        ...
        if ($status_updated) {
            ...
            $recovery_actions[] = __("同步文章任务的subtask_status", 'yali-ai-writer');
            $inconsistencies_found = true;
        }
    ...
    public function requeue_failed_subtasks($task_id, $task_type = 'topic_task', $specific_subtask_ids = null) {
        ...
        if (empty($failed_subtasks)) {
            return array('success' => true, 'requeued_count' => 0, 'message' => __('没有找到需要重新排队的失败子任务', 'yali-ai-writer'));
        }
            ...
            if ($result !== false) {
                ...
                if ($this->logger) {
                    $this->logger->log_success('SUBTASK_REQUEUED', 
                        sprintf(__("子任务重新排队: %1$s, 重试次数: %2$d", 'yali-ai-writer'), $subtask['subtask_id'], ($subtask['retry_count'] + 1)),
        ...
        return array(
            ...
            'message' => sprintf(__("成功重新排队 %d 个失败的子任务", 'yali-ai-writer'), $requeued_count)
        );
    }
    ...
    private function update_parent_task_after_requeue($task_id, $task_type) {
        ...
        if ($this->logger) {
            $this->logger->log_success('PARENT_TASK_UPDATED_AFTER_REQUEUE', 
                sprintf(__("父任务状态已更新为pending，剩余失败子任务: %d", 'yali-ai-writer'), $failed_count),
