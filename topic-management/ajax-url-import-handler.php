...
function cam_process_url_import($url, $provided_content = '', $provided_title = '') {
    ...
    if (!empty($provided_content)) {
        ...
        if (is_wp_error($response)) {
             return ['success' => false, 'data' => ['message' => __('抓取URL失败: ', 'yali-ai-writer') . $response->get_error_message()]];
        }
        
        $response_code = wp_remote_retrieve_response_code($response);
        if ($response_code !== 200) {
             return ['success' => false, 'data' => ['message' => __('抓取URL失败, 状态码: ', 'yali-ai-writer') . $response_code]];
        }
        
        $raw_html = wp_remote_retrieve_body($response);
        if (empty($raw_html)) {
             return ['success' => false, 'data' => ['message' => __('URL返回内容为空', 'yali-ai-writer')]];
        }
        ...
        if (preg_match('/<title[^>]*>(.*?)<\/title>/is', $raw_html, $matches)) {
         ...
         return ['success' => false, 'data' => ['message' => __('未提供URL或内容', 'yali-ai-writer')]];
    }
    ...
    return [
        ...
        'data' => [
            ...
            'url' => $url,
            'message' => __('抓取并优化成功。', 'yali-ai-writer')
        ]
    ];
}
