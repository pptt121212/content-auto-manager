...
class Yali_AI_Writer_TaskStatusManager {
    ...
    public function __construct($database = null, $logger = null) {
        ...
        $this->status_labels = array(
            'pending' => __('待处理', 'yali-ai-writer'),
            'running' => __('运行中', 'yali-ai-writer'),
            'processing' => __('处理中', 'yali-ai-writer'),
            'completed' => __('已完成', 'yali-ai-writer'),
            'failed' => __('失败', 'yali-ai-writer'),
    ...
    public function get_status_label($status) {
        ...
        if (isset($this->status_labels[$normalized])) {
            return $this->status_labels[$normalized];
        }
        return $normalized;
    }
    ...
    public function ensure_status_consistency($task_id, $task_type = 'topic_task') {
        ...
        if ($task['status'] !== $normalized_status) {
            $inconsistencies[] = sprintf(__('主任务状态不规范: \'%1$s\' → \'%2$s\'', 'yali-ai-writer'), $task['status'], $normalized_status);
        }
                ...
                foreach ($subtask_status as $topic_id => $status) {
                    if (is_string($status)) {
                        ...
                        if ($status !== $normalized_subtask) {
                            $inconsistencies[] = sprintf(__('文章子任务[主题ID:%1$s]状态不规范: \'%2$s\' → \'%3$s\'', 'yali-ai-writer'), $topic_id, $status, $normalized_subtask);
                        }
                        ...
                        if ($status['status'] !== $normalized_subtask) {
                            $inconsistencies[] = sprintf(__('文章子任务[主题ID:%1$s]状态不规范: \'%2$s\' → \'%3$s\'', 'yali-ai-writer'), $topic_id, $status['status'], $normalized_subtask);
                        }
                    }
                }
            }
                ...
                foreach ($subtask_status as $index => $subtask) {
                    if (isset($subtask['status'])) {
                        ...
                        if ($subtask['status'] !== $normalized_subtask) {
                            $inconsistencies[] = sprintf(__('子任务[%1$s]状态不规范: \'%2$s\' → \'%3$s\'', 'yali-ai-writer'), $index, $subtask['status'], $normalized_subtask);
                        }
                    }
                }
            }
        ...
        foreach ($queue_items as $queue_item) {
            ...
            if ($queue_item['status'] !== $normalized_queue) {
                $inconsistencies[] = sprintf(__('队列项[%1$s]状态不规范: \'%2$s\' → \'%3$s\'', 'yali-ai-writer'), $queue_item['id'], $queue_item['status'], $normalized_queue);
            }
        }
    ...
    private function check_article_specific_consistency($task, $queue_items) {
        ...
        if (!is_array($topic_ids)) {
            $inconsistencies[] = __("文章任务的topic_ids字段格式错误", 'yali-ai-writer');
        ...
        if (!empty($missing_in_queue)) {
            $inconsistencies[] = __("队列中缺失主题ID: ", 'yali-ai-writer') . implode(', ', $missing_in_queue);
        }
        
        // 检查是否有队列项的reference_id不在主题列表中
        $extra_in_queue = array_diff($queue_reference_ids, $topic_ids);
        if (!empty($extra_in_queue)) {
            $inconsistencies[] = __("队列中存在多余的主题ID: ", 'yali-ai-writer') . implode(', ', $extra_in_queue);
        }
        ...
        if ($task['completed_topics'] != $completed_count) {
            $inconsistencies[] = sprintf(__("已完成主题计数不一致: 任务记录=%1$d, 队列统计=%2$d", 'yali-ai-writer'), $task['completed_topics'], $completed_count);
        }
        
        if ($task['failed_topics'] != $failed_count) {
            $inconsistencies[] = sprintf(__("失败主题计数不一致: 任务记录=%1$d, 队列统计=%2$d", 'yali-ai-writer'), $task['failed_topics'], $failed_count);
        }
        ...
        if ($total_processed > $total_topics) {
            $inconsistencies[] = sprintf(__("处理的主题数超过总主题数: 处理=%1$d, 总数=%2$d", 'yali-ai-writer'), $total_processed, $total_topics);
        }
        
        // 检查任务状态与子任务状态的一致性
        if ($task['status'] === 'completed' && $failed_count > 0) {
            $inconsistencies[] = __("任务状态为已完成但存在失败的子任务", 'yali-ai-writer');
        }
        
        if ($task['status'] === 'completed' && $total_processed < $total_topics) {
            $inconsistencies[] = __("任务状态为已完成但仍有未处理的子任务", 'yali-ai-writer');
        }
