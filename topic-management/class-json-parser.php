...
class ContentAuto_JsonParser {
    ...
    private function repair_json_format($content) {
        ...
        // 修复常见的引号问题（将中文全角引号替换为英文半角引号）
        $repaired = str_replace('“', '"', $repaired);
        $repaired = str_replace('”', '"', $repaired);
        $repaired = str_replace('‘', "'", $repaired);
        $repaired = str_replace('’', "'", $repaired);
        
        // 修复常见的标点符号问题（将中文全角标点替换为英文半角标点）
        $repaired = str_replace('：', ':', $repaired);
        $repaired = str_replace('，', ',', $repaired);
