...
function get_rule_type_name($rule) {
    ...
    return Yali_AI_Writer_RuleManager::get_rule_type_label($rule->rule_type);
}
?>
...
<div class="wrap yali-plugin-wrapper">
    ...
    <div class="yali-card">
        ...
        <form method="post" action="">
            <?php wp_nonce_field('yali_ai_writer_manager_topic_jobs', 'yali_ai_writer_manager_nonce'); ?>
            ...
            <table class="form-table">
                        ...
                        <?php else: ?>
                            ...
                            <div id="browser-extension-notice" class="yali-notice yali-notice-info" style="display: none;">
                                ...
                                <p>
                                    ...
                                    <strong><?php _e('请确保已安装并开启：', 'yali-ai-writer'); ?><a href="https://www.yaliai.com/product/extension/" target="_blank" rel="noopener" class="yali-link"><?php _e('鸭梨AI浏览器扩展', 'yali-ai-writer'); ?></a></strong>
                                </p>
                            </div>
                ...
                <tr>
                    ...
                    <td>
                        <input type="number" name="topic_count" value="<?php echo YALI_AI_WRITER_DEFAULT_TOPIC_COUNT; ?>" min="1" max="100" class="small-text yali-input" required <?php echo empty($rules) ? 'disabled' : ''; ?>>
                    </td>
                </tr>
            </table>
            ...
            <table class="wp-list-table widefat striped yali-table topic-jobs-table">
                ...
                <tbody>
                    <?php foreach ($tasks as $task): ?>
                        <?php
                        // 获取规则信息
                        $rule = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}yali_ai_writer_rules WHERE id = %d", $task['rule_id']));
                        ...
                        <tr class="task-row <?php echo $task['status'] === 'failed' ? 'failed-task' : ''; ?>" data-task-id="<?php echo esc_attr($task['topic_task_id']); ?>">
                            ...
                            <td>
                                ...
                                <?php if ($task['status'] === YALI_AI_WRITER_STATUS_FAILED): ?>
                                ...
                                <div class="task-controls" data-task-id="<?php echo esc_attr($task['topic_task_id']); ?>">
                                    ...
                                    <?php if (in_array($task['status'], ['pending', 'running', 'processing'])) : ?>
                                        <button class="yali-btn-small yali-btn-secondary" type="button" 
                                                ...
                                                data-yali-ajax-action="yali_ai_writer_pause_task" 
                                    ...
                                    <?php if ($task['status'] === 'paused') : ?>
                                        <button class="yali-btn-small yali-btn-secondary" type="button" 
                                                data-yali-action="ajax" 
                                                data-yali-ajax-action="yali_ai_writer_resume_task" 
                                                data-yali-param-task_id="<?php echo esc_attr($task['topic_task_id']); ?>" 
                                                data-yali-reload="true" 
                                    ...
                                    <?php if ($task['status'] === 'failed') : ?>
                                        <button class="yali-btn-small yali-btn-secondary" type="button" 
                                                data-yali-action="ajax" 
                                                data-yali-ajax-action="yali_ai_writer_retry_task" 
                                                data-yali-param-task_id="<?php echo esc_attr($task['topic_task_id']); ?>" 
                                                data-yali-reload="true" 
                                    ...
                                    <?php if ($task['status'] !== 'completed') : ?>
                                        <button class="yali-btn yali-btn-small yali-btn-danger" type="button" 
                                                ...
                                                data-yali-ajax-action="yali_ai_writer_delete_task"
