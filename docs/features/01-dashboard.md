...
## 业务逻辑
1. **统计聚合**：
   - 调用 `ContentAuto_Database::get_dashboard_stats()` 获取系统全局汇总数据：主题总数、文章总数、已发文章、总字符数、活跃规则、高优先级主题等。
   - 通过 `ContentAuto_JobQueue::get_queue_status()` 获取任务队列各状态（待处理、处理中、已完成）及各任务类型（主题生成、文章生成、素材搜索、向量处理、图片生成）的精细分布。
   - 调用 `get_vector_generation_stats()` 获取向量处理细分统计（待处理、处理中、已完成、失败）。
2. **多维度概览卡片**：
   - **核心指标**：展示 API 配置状态、日均文章产出、系统整体成功率。
   - **主题/文章任务**：饼状图或卡片展示任务完成进度与失败率趋势。
   - **向量覆盖率**：实时计算已向量化主题比例，衡量相似度推荐和智能分类的基础。
   - **自动配图情况**：统计已自动配图的文章数量及生成的图片总数。
3. **分类与向量管理**：
   - 提供刷新分类向量缓存（Category Cache）的操作，触发 `content_auto_refresh_category_cache()` 计算最子级分类的语义特征，支持手工主题的智能自动归类。
   - 监控分类覆盖情况，确保每一个最末级分类都被纳入向量化体系。
4. **系统状态雷达/监控**：
   - 实时指示任务处理器（队列）活跃状态。
   - API 健康度监控：活跃 API 数量、支持向量的 API 数量。
   - 内容质量监控：平均处理时间、成功率仪表盘。
5. **任务管理与干预**：
   - 提供清除任务队列功能，通过 AJAX 操作 `content_auto_clear_task_queue` 一键重置阻塞或失败的任务。
