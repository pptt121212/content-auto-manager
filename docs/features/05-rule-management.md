...
## 业务逻辑
1. **规则定义与数据源类型**：
   - 系统将"规则"定义为生成内容的数据来源与提取逻辑。支持 5 种核心来源类型：
     - **随机选择文章 (random_selection)**：从选定的 WordPress 分类中随机抓取存量文章，作为生成新内容的上下文。
     - **固定选择文章 (fixed_articles)**：精准指定某几篇历史文章作为创作蓝本。
     - **上传文本内容 (upload_text)**：直接粘贴外部参考资料作为主题生成的输入。
     - **导入关键词 (import_keywords)**：按行导入关键词，系统将为每个关键词生成独立主题。
     - **随机分类 (random_categories)**：随机从选定分类库中提取分类名称/描述，用于生成该领域的通用/探索性主题。
2. **扩展配置项**：
   - **目标分类 (Target Category)**：所有规则均可指定强制的目标分类。若未指定，AI 将根据内容智能匹配最相似的分类。
   - **参考资料 (Reference Material)**：支持录入最多 800 字的背景信息（如品牌调性、专业知识库），在文章生成阶段作为 AI 的核心约束。
   - **规则循环次数 (Item Count)**：定义该规则单次任务执行时生成的主题/文章数量。
3. **规则锁定机制**：
   - 调用 `is_rule_in_use()` 检查规则是否正关联在"待处理"或"运行中"的主题任务队列中。
   - 活跃任务关联的规则禁止修改或删除，确保生产一致性。
4. **验证与存储**：
   - `validate_rule_data()` 负责清洗和截取（防止长文本溢出）。内容存储于 `rules` 主表，多对多分类关系存储于 `rule_items`。
5. **任务级联**：
   - 当主题任务执行时，系统通过 `get_content_by_rule_for_subtask()` 根据规则 ID 和子任务索引精准还原生成背景。
