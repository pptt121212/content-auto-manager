...
class Yali_AI_Writer_XmlTemplateProcessor {
    ...
    public function generate_prompt($topic_data, $publish_rules, $related_content = array(), $similar_articles = array()) {
        ...
        $image_prompt_mode = isset($publish_rules['image_prompt_mode']) ? $publish_rules['image_prompt_mode'] : 'default';
        ...
        if ($is_rewrite_task) {
            // 仿写模式：绝对不使用任何文章结构，包括AI生成的动态结构
            $structure_info = ['id' => null, 'name' => '', 'sections' => ''];
        } elseif (!empty($topic_data['dynamic_structure_block'])) {
            // Check if dynamic structure was successfully generated in step 1
            $structure_info = [
                'id' => null, // AI generated structures don't have DB IDs
                'name' => 'AI Generated (' . $topic_data['source_angle'] . ')',
                'sections' => str_replace(['<structure>', '</structure>'], '', $topic_data['dynamic_structure_block'])
            ];
            $debug_logger->info('[TEMPLATE_DEBUG] Using AI-generated dynamic structure block', ['length' => strlen($topic_data['dynamic_structure_block'])]);
        } else {
        ...
        // --- ENHANCEMENT: Allow overriding template for structure generation ---
        $template_override = null;
        if (func_num_args() > 4) {
            $template_override = func_get_arg(4);
        }
    ...
    private function validate_input_data($topic_data, $publish_rules) {
        ...
        // 验证目标字数
        $validated['target_length'] = isset($publish_rules['target_length']) ? trim($publish_rules['target_length']) : '不少于2000字';
        $valid_lengths = array('不少于500字', '不少于800字', '不少于2000字', '不少于5000字', '不少于8000字', '不少于12000字');
        // 兼容旧区间格式的历史数据，自动映射到最近的新档位
        $legacy_map = array(
            '300-800'   => '不少于500字',
            '500-1000'  => '不少于800字',
            '800-1500'  => '不少于2000字',
        ...
        if (isset($legacy_map[$validated['target_length']])) {
            $validated['target_length'] = $legacy_map[$validated['target_length']];
        }
        if (!in_array($validated['target_length'], $valid_lengths)) {
            $validated['target_length'] = '不少于2000字';
        }
    ...
    private function build_xml_prompt($topic_data, $title, $source_angle, $user_value, $seo_keywords, $matched_category, $target_length, $knowledge_depth, $reader_role, $normalize_output, $auto_image_insertion, $enable_internal_linking, $publish_language, $structure_template, $structure_sections, $source_materials_content, $similar_articles_content, $reference_material = '', $image_prompt_template = '', $is_rewrite_task_override = false, $template_override = null) {
        ...
        if ($template_override) {
            ...
            // 使用插件自身的语言设置（而非 WordPress 站点语言）决定模板语言
            $plugin_locale = get_option('yali_ai_writer_locale', 'site_default');
            $locale = ($plugin_locale && $plugin_locale !== 'site_default') ? $plugin_locale : (function_exists('get_user_locale') ? get_user_locale() : get_locale());
            $is_non_chinese = strpos($locale, 'zh') !== 0;

            if ($is_non_chinese && file_exists(__DIR__ . '/en/' . $selected_template)) {
                $template_path = __DIR__ . '/en/' . $selected_template;
            }
        ...
        // 如果是仿写模式，强制使用仿写模板
        if ($is_rewrite_task && empty($template_override)) {
  ...
  <internal_linking_instructions>
    ...
    ' . $similar_articles_content . '
        ...
        if ($is_non_chinese) {
            $target_length_en_map = array(
                '不少于500字'   => 'at least 500 words',
                '不少于800字'   => 'at least 800 words',
                '不少于2000字'  => 'at least 2,000 words',
                '不少于5000字'  => 'at least 5,000 words',
                '不少于8000字'  => 'at least 8,000 words',
                '不少于12000字' => 'at least 12,000 words',
            );
            if (isset($target_length_en_map[$target_length])) {
                $localized_target_length = $target_length_en_map[$target_length];
            }
        }
        ...
        $replacements = array(
            ...
            '{{TARGET_LENGTH}}' => $localized_target_length,
