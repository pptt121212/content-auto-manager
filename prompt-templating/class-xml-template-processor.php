...
class ContentAuto_XmlTemplateProcessor {
     ...
     * 构建XML模板提示词
     */
    private function build_xml_prompt($title, $source_angle, $user_value, $seo_keywords, $matched_category, $target_length, $knowledge_depth, $reader_role, $normalize_output, $auto_image_insertion, $enable_internal_linking, $publish_language, $structure_template, $structure_sections, $source_materials_content, $similar_articles_content, $reference_material = '') {
  ...
  <internal_linking_instructions>
    ...
    ' . $similar_articles_content . '
        ...
        if (defined('CONTENT_AUTO_DEBUG_MODE') && CONTENT_AUTO_DEBUG_MODE) {
            $this->log_article_prompt_to_file($prompt, $topic_data);
        }
     ...
     * 记录文章生成提示词到统一日志系统
     */
    private function log_article_prompt_to_file($prompt_content, $topic_data) {
        try {
            // 仅在调试模式下记录完整提示词
            if (defined('CONTENT_AUTO_DEBUG_MODE') && CONTENT_AUTO_DEBUG_MODE) {
                // 引入日志系统
                ...
                $context = array(
                    'type' => 'ARTICLE_PROMPT',
                    'title' => isset($topic_data['title']) ? $topic_data['title'] : '未知标题',
                    'source_angle' => isset($topic_data['source_angle']) ? $topic_data['source_angle'] : '未知角度',
            ...
            // 静默处理错误，避免影响主流程
            error_log('文章提示词日志记录失败: ' . $e->getMessage());
        }
    }
