...
class Yali_AI_Writer_RuleHandler {
     ...
     * 处理规则保存的 AJAX 请求
     */
    public function handle_ajax_save_rule() {
        // 复用 handle_form_submission 的逻辑，但通过参数控制返回 JSON
        $this->process_save_rule(true);
    }

    /**
     * 处理规则添加/编辑表单的提交 (传统 POST)
     */
    public function handle_form_submission() {
        // 防止 AJAX 请求触发此逻辑 (避免 wp_ajax_ 钩子被劫持)
        if (wp_doing_ajax()) {
            return;
        }
    ...
    /**
     * 核心保存逻辑
     * @param bool $is_ajax 是否为 AJAX 请求
     */
    private function process_save_rule($is_ajax = false) {
        // 验证 nonce (如果是 AJAX，nonce 可能在不同字段，需统一检查)
        $nonce = isset($_POST['cam_save_rule_nonce']) ? $_POST['cam_save_rule_nonce'] : (isset($_POST['nonce']) ? $_POST['nonce'] : '');
        if (!wp_verify_nonce($nonce, 'cam_save_rule_action')) {
            if ($is_ajax) {
        ...
        if (!current_user_can('manage_options')) {
            if ($is_ajax) {
                ...
                wp_die(__('您没有权限执行此操作。', 'yali-ai-writer'));
            }
        }
        ...
        if ($is_edit_mode) {
            ...
            if ($rule_manager->is_rule_in_use($rule_id)) {
                // 规则正在使用中，不允许修改
                if ($is_ajax) {
                    wp_send_json_error(array('message' => __('规则正在使用中，无法修改。', 'yali-ai-writer')));
                } else {
                     wp_redirect(admin_url('admin.php?page=yali-ai-writer-rules&message=6'));
                     exit;
                }
            }
        ...
        if ($wpdb->last_error) {
             if ($is_ajax) {
                 wp_send_json_error(array('message' => __('数据库错误: ', 'yali-ai-writer') . $wpdb->last_error));
             }
        }

        if ($result === false) {
             if ($is_ajax) {
                 wp_send_json_error(array('message' => __('保存失败，请重试。', 'yali-ai-writer')));
             }
        }

        if (!$is_edit_mode && !$rule_id) {
            if ($is_ajax) {
                 wp_send_json_error(array('message' => __('创建规则失败，ID未生成。', 'yali-ai-writer')));
            } else {
                wp_redirect(admin_url('admin.php?page=yali-ai-writer-rules&message=2'));
                exit;
            }
        }
        ...
        // 4. 操作完成
        if ($is_ajax) {
             $message = $is_edit_mode ? __('规则已更新。', 'yali-ai-writer') : __('规则创建成功！正在跳转...', 'yali-ai-writer');
             $type = $is_edit_mode ? 'update' : 'create';
             $redirect_url = admin_url('admin.php?page=yali-ai-writer-rules&message=' . ($is_edit_mode ? 3 : 1));
            ...
            $message_code = $is_edit_mode ? 3 : 1; 
            wp_redirect(admin_url('admin.php?page=yali-ai-writer-rules&message=' . $message_code));
            exit;
        }
    }
    ...
    public function handle_delete_rule() {
        ...
        if ($rule_id <= 0) {
            wp_send_json_error(array('message' => __('无效的规则ID。', 'yali-ai-writer')));
        }
