    ...
    if (isset($_GET['message'])) {
        ...
        if ($dedup_info && ($message_code === 1 || $message_code === 3)) {
            ...
            if ($total > 0) {
                ...
                echo '<p><strong>' . __('⚠️ 网址去重提示：', 'yali-ai-writer') . '</strong>' . sprintf( __('共过滤了 %s 条重复网址', 'yali-ai-writer'), '<strong>' . $total . '</strong>' ) . '</p>';
                
                // 1. 输入中的重复
                if ($count_input > 0) {
                    echo '<div style="margin: 10px 0; padding: 10px; background: #fff3cd; border-radius: 4px;">';
                    echo '<strong>' . sprintf( __('📋 输入重复（%d 条）', 'yali-ai-writer'), $count_input ) . '</strong>';
                    echo '<p style="margin: 5px 0 0; color: #856404; font-size: 12px;">' . __('在您输入的网址中发现重复项，已自动保留第一条', 'yali-ai-writer') . '</p>';
                    if (!empty($dedup_info['urls_in_input'])) {
                        ...
                        if ($count_input > 10) {
                            echo '<li>' . sprintf( __('...还有 %d 条', 'yali-ai-writer'), ($count_input - 10) ) . '</li>';
                        }
                        echo '</ul>';
                    }
                    echo '</div>';
                ...
                if ($count_rules > 0) {
                    ...
                    echo '<strong>' . sprintf( __('📁 其他规则中已存在（%d 条）', 'yali-ai-writer'), $count_rules ) . '</strong>';
                    echo '<p style="margin: 5px 0 0; color: #004085; font-size: 12px;">' . __('这些网址已在其他规则中等待处理，无需重复添加', 'yali-ai-writer') . '</p>';
                    if (!empty($dedup_info['urls_in_rules'])) {
                        ...
                        if ($count_rules > 10) {
                            echo '<li>' . sprintf( __('...还有 %d 条', 'yali-ai-writer'), ($count_rules - 10) ) . '</li>';
                        }
                        echo '</ul>';
                    }
                    echo '</div>';
                ...
                if ($count_topics > 0) {
                    ...
                    echo '<strong>' . sprintf( __('✅ 已生成过主题（%d 条）', 'yali-ai-writer'), $count_topics ) . '</strong>';
                    echo '<p style="margin: 5px 0 0; color: #155724; font-size: 12px;">' . __('这些网址之前已成功生成过主题内容，无需重复采集', 'yali-ai-writer') . '</p>';
                    if (!empty($dedup_info['urls_in_topics'])) {
                        ...
                        if ($count_topics > 10) {
                            echo '<li>' . sprintf( __('...还有 %d 条', 'yali-ai-writer'), ($count_topics - 10) ) . '</li>';
                        }
                        echo '</ul>';
                    }
                    echo '</div>';
    ...
    <div class="yali-card">
        <div class="yali-card-header">
            ...
            <div class="yali-card-actions">
                <a href="<?php echo admin_url('admin.php?page=yali-ai-writer-rules&action=add'); ?>" class="yali-btn yali-btn-primary">
                ...
                <?php else : ?>
                    <?php foreach ($rules as $rule) : ?>
                        <tr<?php echo $rule->in_use ? ' style="background-color: #fff3cd;"' : ''; ?>>
                            ...
                            <td><?php echo $rule->status == 1 ? '<span class="yali-badge yali-badge-success">' . __('启用', 'yali-ai-writer') . '</span>' : '<span class="yali-badge yali-badge-neutral">' . __('禁用', 'yali-ai-writer') . '</span>'; ?></td>
                            <td>
                                <?php if ($rule->in_use): ?>
                                    <span class="yali-badge yali-badge-warning"><?php _e('使用中', 'yali-ai-writer'); ?></span>
                                    <div style="font-size: 11px; color: #666; margin-top: 4px;">
                                        <?php printf( __('主题任务: %d 个', 'yali-ai-writer'), $rule->usage_topic_tasks ); ?>
                                    </div>
                                <?php else: ?>
                            ...
                            <td>
                                ...
                                <?php else: ?>
                                    <a href="<?php echo admin_url('admin.php?page=yali-ai-writer-rules&action=edit&id=' . $rule->id); ?>" class="yali-btn yali-btn-small"><?php _e('编辑', 'yali-ai-writer'); ?></a>
                                    <button type="button" class="yali-btn yali-btn-small yali-btn-danger" 
                                            data-yali-action="delete" 
                                            data-yali-ajax-action="content_auto_delete_rule" 
...
<script>
// i18n variables are defined dynamically here
window.camRulesListI18n = {
    addSuccess: '<?php _e('新规则已成功添加', 'yali-ai-writer'); ?>',
    saveError: '<?php _e('错误：规则保存失败，请重试', 'yali-ai-writer'); ?>',
    updateSuccess: '<?php _e('规则已成功更新', 'yali-ai-writer'); ?>',
    deleteSuccess: '<?php _e('规则已成功删除', 'yali-ai-writer'); ?>',
    deleteError: '<?php _e('删除规则失败，请重试', 'yali-ai-writer'); ?>',
    confirmDelete: '<?php _e('确定要删除规则 "%s" 吗？此操作不可撤销。', 'yali-ai-writer'); ?>',
    networkError: '<?php _e('网络请求失败', 'yali-ai-writer'); ?>',
    deleteErrorGeneral: '<?php _e('删除规则时发生错误', 'yali-ai-writer'); ?>',
    ruleInUseEdit: '<?php _e('此规则正在被使用中，无法编辑。请等待所有相关任务完成后再试。', 'yali-ai-writer'); ?>',
    ruleInUseDelete: '<?php _e('此规则正在被使用中，无法删除。请等待所有相关任务完成后再试。', 'yali-ai-writer'); ?>'
};
</script>
