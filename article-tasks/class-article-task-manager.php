...
class ContentAuto_ArticleTaskManager {
    ...
    public function create_article_task($topic_ids, $name = '') {
        ...
        if (empty($name)) {
            $name = sprintf(__('Article Task Group - %s (%d topics)', 'yali-ai-writer'), date('Y-m-d H:i:s', strtotime($current_time)), $total_topics);
        }
            ...
            if ($lock_success) {
                ...
                if ($queue_success) {
                    $this->logger->log_success('TASK_CREATED', 
                        sprintf(__('Article task group created: %s, containing %d topic(s)', 'yali-ai-writer'), $name, $total_topics),
                        $success_context
                    );
    ...
    private function process_current_topic_task($task, $subtask_id) {
        ...
        if ($result['success']) {
            // [增强] 保存文章前更新心跳
            $this->update_task_heartbeat($task['id'], $subtask_id, 'BEFORE_CREATE_POST');
            
            $this->logger->log_success('TOPIC_TASK_PROCESSING_SUCCESS', '主题任务处理成功', $base_context);
            
            // [增强] 保存文章后更新心跳
            $this->update_task_heartbeat($task['id'], $subtask_id, 'AFTER_CREATE_POST');
    ...
    private function handle_failed_processing($task_id, $task, $subtask_id, $error_details) {
        ...
        if (!$is_api_error) {
            ...
            return sprintf(__('子任务 %s 处理失败（非API错误，直接标记为最终失败）', 'yali-ai-writer'), $subtask_id);
        }
        ...
        // 3. 解锁相关主题（防止僵尸主题）
        // 从 database 获取 reference_id (topic_id)
        $queue_record = $wpdb->get_row($wpdb->prepare(
            "SELECT reference_id FROM {$queue_table} WHERE job_type = 'article' AND job_id = %d AND subtask_id = %s",
            $task_id, $subtask_id
        ));
    ...
    private function mark_final_failure($task_id, $subtask_id, $error_details) {
        ...
        $error_detail_str = is_array($error_details['message']) ? json_encode($error_details['message'], JSON_UNESCAPED_UNICODE) : $error_details['message'];
        $error_message = sprintf(
            __('子任务 %1$s 最终失败（非API错误）. 阶段: %2$s, 详情: %3$s', 'yali-ai-writer'),
            $subtask_id,
            $error_stage,
            $error_detail_str
        );
