...
class ContentAuto_ArticleTaskManager {
    ...
    public function create_article_task($topic_ids, $name = '') {
        ...
        if ($task_id) {
            ...
            // 锁定主题状态为queued
            $this->logger->log_info('TOPIC_LOCKING', "开始锁定主题状态", $success_context);
            $lock_success = $this->lock_topics_for_task($topic_ids);
            
            if ($lock_success) {
                $this->logger->log_success('TOPIC_LOCKED', "主题状态锁定成功", $success_context);
    ...
    private function process_current_topic_task($task, $subtask_id) {
        ...
        if (!$queue_record || !$queue_record->reference_id) {
            ...
            $this->logger->log_error('QUEUE_FETCH', '无法获取队列信息中的主题ID');
        ...
        if (!$topic) {
            ...
            $this->logger->log_error('TOPIC_NOT_FOUND', '主题不存在: ' . $topic_id);
            return ['success' => false, 'error' => $error_details];
        }
