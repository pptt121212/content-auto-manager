...
class ContentAuto_ArticleTaskTimeoutHandler {
    ...
    public function handle_timeout_tasks() {
        ...
        $absolute_timeout = 1800;  // 绝对超时: 30分钟
    ...
    private function handle_timeout_subtask($subtask) {
        ...
        try {
            ...
            // 检查是否为API相关的超时
        $is_api_timeout = in_array($business_node_info['node'], ['API请求阶段', '预定义API请求阶段', '自定义API请求阶段', 'API重试阶段']);
            ...
            if ($is_api_timeout) {
                ...
                __('API请求失败，重试%d次后超时: 子任务ID=%s, 任务ID=%d, 处理时长=%d秒, 业务节点=%s, 详细信息=%s', 'yali-ai-writer'),
                $display_retry_count,
                $subtask['subtask_id'],
                $subtask['job_id'],
                ...
                $error_message = sprintf(
                __('子任务处理超时 (超过6分钟): 子任务ID=%s, 任务ID=%d, 开始处理时间=%s, 处理时长=%d秒, 业务节点=%s, 详细信息=%s', 'yali-ai-writer'),
                $subtask['subtask_id'],
                $subtask['job_id'],
                $subtask['updated_at'],
                $subtask['processing_seconds'],
