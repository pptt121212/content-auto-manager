    ...
    private function execute_api_request_with_retry($prompt, $topic, $max_retries = 3, $task_id = null, $subtask_id = null) {
        ...
        for ($attempt = 1; $attempt <= $max_retries; $attempt++) {
            ...
            if ($result['success']) {
                ...
                $this->logger->log_success('API_SUCCESS', "API请求成功", $api_context);
    ...
    private function make_api_request($api_config, $prompt, $topic, $attempt = 1, $max_retries = 3) {
        try {
            ...
            if (isset($response_data['choices'][0]['message']['content'])) {
                ...
                // 仅在调试模式下记录API原始响应格式
                if (!empty($raw_content) && defined('CONTENT_AUTO_DEBUG_MODE') && CONTENT_AUTO_DEBUG_MODE) {
                    $this->logger->log_debug('API_RAW_RESPONSE', 'API原始响应内容:', [
                        'topic_id' => $topic['id'],
                        'topic_title' => $topic['title'],
                        'response_type' => gettype($raw_content)
                    ]);
                }
