...
class ContentAuto_StructureExtractor {
    ...
    public function generalize_structure($extracted_structure, $content_angle) {
        ...
        // 调用AI API
        $result_data = $this->api_handler->generate_content($prompt, 'structure_generation', array(
            'content_angle' => $content_angle,
            'source_article_id' => $extracted_structure['post_id'],
            'return_usage' => true // 加入 return_usage 用于检测是否被截断
        ));
        
        // 检查错误（finish_reason == 'length' 已由底层 UnifiedApiHandler 自动触发 API 轮询切换）
        ...
        $result_text = is_array($result_data) && isset($result_data['content']) ? $result_data['content'] : $result_data;
        
        // 解析AI返回的JSON
        $parsed = $this->parse_ai_response($result_text);
        
        if (!$parsed) {
            $this->log_error('GENERALIZE_FAILED', 'AI响应解析失败', array(
                'content_angle' => $content_angle,
                'response' => substr($result_text, 0, 500)
            ));
            return false;
        }
