...
class Yali_AI_Writer_DataValidator {
    ...
    public function validate_field($field_name, $field_value, $rules) {
        ...
        if (isset($rules['required']) && $rules['required'] && empty($field_value)) {
            $errors[] = sprintf('字段 %s 是必填的', $field_name);
            return $errors;
        }
            ...
            switch ($rules['type']) {
                case 'integer':
                    if (!is_int($field_value) && !is_numeric($field_value)) {
                        $errors[] = sprintf('字段 %s 必须是整数', $field_name);
                    } else {
                        $field_value = intval($field_value);
                        
                        // 最小值验证
                        if (isset($rules['min']) && $field_value < $rules['min']) {
                            $errors[] = sprintf('字段 %s 不能小于 %d', $field_name, $rules['min']);
                        }
                        
                        // 最大值验证
                        if (isset($rules['max']) && $field_value > $rules['max']) {
                            $errors[] = sprintf('字段 %s 不能大于 %d', $field_name, $rules['max']);
                        }
                    }
                    break;
                ...
                case 'string':
                    if (!is_string($field_value)) {
                        $errors[] = sprintf('字段 %s 必须是字符串', $field_name);
                    } else {
                        // 枚举值验证
                        if (isset($rules['enum']) && !in_array($field_value, $rules['enum'])) {
                            $errors[] = sprintf('字段 %s 必须是以下值之一: %s', $field_name, implode(', ', $rules['enum']));
                        }
                        
                        // 最大长度验证
                        if (isset($rules['max_length']) && strlen($field_value) > $rules['max_length']) {
                            $errors[] = sprintf('字段 %s 长度不能超过 %d 个字符', $field_name, $rules['max_length']);
                        }
                    }
                    break;
                ...
                case 'array':
                    if (!is_array($field_value)) {
                        $errors[] = sprintf('字段 %s 必须是数组', $field_name);
