...
class Yali_AI_Writer_VectorApiHandler {
    ...
    public function generate_embeddings_batch(array $texts, $config_id = null) {
        ...
        try {
            ...
            if (!$api_config) {
                $this->last_error = __('没有可用的向量API配置', 'yali-ai-writer');
            ...
            $this->log_error('EMBEDDING_BATCH_EXCEPTION', __('批量生成文本嵌入向量时发生异常: ', 'yali-ai-writer') . $e->getMessage(), array_merge($context, [
    ...
    private function check_rate_limit($api_config) {
        ...
        if (!$this->rate_limiter->can_proceed($key, $rpm, 'requests')) {
            $this->last_error = __('超过RPM限制，请稍后再试', 'yali-ai-writer');
        ...
        if (!$this->rate_limiter->can_proceed($key . '_tokens', $tpm, 'tokens')) {
            $this->last_error = __('超过TPM限制，请稍后再试', 'yali-ai-writer');
    ...
    private function send_http_request($api_config, $request_data) {
        ...
        if (is_wp_error($response)) {
            ...
            if (strpos($error_message, 'Operation timed out') !== false || strpos($error_message, 'timeout') !== false) {
                ...
                $this->last_error = sprintf(
                    __('API请求超时（%d秒）。%s API服务器可能响应较慢，这通常是正常现象。建议稍后重试，或检查网络连接。', 'yali-ai-writer'),
                    $timeout,
        ...
        if ($response_code !== 200) {
            $this->last_error = __('HTTP响应错误: ', 'yali-ai-writer') . $response_code;
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->last_error = __('JSON解析错误: ', 'yali-ai-writer') . json_last_error_msg();
    ...
    private function parse_response($response, $api_config) {
        ...
        if (isset($response['error']) || isset($response['detail']) || isset($response['message'])) {
            ...
            $this->last_error = __('API返回错误: ', 'yali-ai-writer') . $error_msg;
        ...
        $this->last_error = __('无法识别的响应格式: 既无data也无embeddings字段', 'yali-ai-writer');
    ...
    private function parse_openai_response($response, $api_config) {
        if (!isset($response['data']) || !is_array($response['data'])) {
            $this->last_error = __('响应格式错误：缺少data字段', 'yali-ai-writer');
        ...
        if (empty($embeddings)) {
            $this->last_error = __('响应中未找到有效的向量数据', 'yali-ai-writer');
    ...
    private function parse_jina_response($response, $api_config) {
        if (!isset($response['embeddings']) || !is_array($response['embeddings'])) {
            $this->last_error = __('响应格式错误：缺少embeddings字段', 'yali-ai-writer');
            $this->log_error('RESPONSE_FORMAT_ERROR', $this->last_error, ['response' => $response]);
            return false;
        }
    ...
    public function test_connection($config_id) {
        ...
        try {
            ...
            if (empty($config_id) || !is_numeric($config_id)) {
                return array(
                    'success' => false,
                    'message' => __('无效的配置ID', 'yali-ai-writer')
                );
            }
            ...
            if (!$config) {
                return array(
                    'success' => false,
                    'message' => __('未找到向量API配置 (ID: ', 'yali-ai-writer') . $config_id . ')'
                );
            }
            ...
            if (empty($config['vector_api_url']) || empty($config['vector_api_key']) || empty($config['vector_model_name'])) {
                return array(
                    'success' => false,
                    'message' => __('配置不是有效的向量API配置 (ID: ', 'yali-ai-writer') . $config_id . ')'
                );
            }
            ...
            if (!filter_var($config['vector_api_url'], FILTER_VALIDATE_URL)) {
                return array(
                    'success' => false,
                    'message' => __('无效的向量API URL: ', 'yali-ai-writer') . $config['vector_api_url']
                );
            }
            ...
            if (!$this->check_rate_limit($config)) {
                return array(
                    'success' => false,
                    'message' => __('超过速率限制，请稍后再试', 'yali-ai-writer')
                );
            }
            ...
            if ($result === false) {
                return array(
                    'success' => false,
                    'message' => __('请求失败：', 'yali-ai-writer') . $this->get_last_error()
                );
            }
            ...
            if ($embedding_data === false) {
                return array(
                    'success' => false,
                    'message' => __('响应解析失败：', 'yali-ai-writer') . $this->get_last_error()
                );
            }
            ...
            if ($first_embedding === null) {
                return array(
                    'success' => false,
                    'message' => __('响应中未找到有效的向量数据', 'yali-ai-writer')
                );
            }
            ...
            $success_message = sprintf(
                __('向量API连接成功！向量维度：%d，模型：%s，响应时间：%.2f秒', 'yali-ai-writer'),
                $dimensions,
            ...
            return array(
                'success' => false,
                'message' => __('测试异常：', 'yali-ai-writer') . $e->getMessage()
            );
        }
    }
}
