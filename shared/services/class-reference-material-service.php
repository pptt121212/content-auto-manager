...
class ContentAuto_ReferenceMaterialService {
    ...
    public function test_brand_profile_recall($topic_data) {
        ...
        if (empty($candidates)) {
            $result['error'] = sprintf(
                '没有召回到任何候选参考资料。当前阈值为 %.1f，共有 %d 条参考资料，但没有一条的相似度达到阈值。请查看下方"所有参考资料相似度"了解详情。',
                self::SIMILARITY_THRESHOLD_AI_SELECT,
                count($reference_profiles)
            );
            $result['debug_info']['suggestion'] = '建议：1) 检查参考资料标题是否与主题相关；2) 或者降低相似度阈值';
            return $result;
        }
    ...
    private function ai_select_best_reference($topic_data, $candidates) {
        ...
        $response = $api_handler->generate_content($prompt, 'reference_select', array(
            'topic_id' => $topic_data['id'] ?? null
        ));
        ...
        // 解析大模型返回的ID
        $selected_id = $this->parse_ai_response($response, $candidates);
