    ...
    try {
        ...
        if ($success) {
            if (!empty($updates_applied)) {
                $message = __('数据库表结构已成功更新。已应用更新： ', 'yali-ai-writer') . implode(", ", $updates_applied);
            } else {
                $message = __('数据库表结构已是最新版本，无需更新。', 'yali-ai-writer');
            }
        } else {
            $message = __('数据库更新过程中出现错误： ', 'yali-ai-writer') . implode("; ", $errors);
        }
        ...
        return array(
            "success" => false,
            "message" => __('数据库更新失败：', 'yali-ai-writer') . $e->getMessage(),
...
function content_auto_search($query, $max_results = null) {
    ...
    // 确定单次搜索最大数量（如果在调用时指定了，则优先使用指定值；否则使用全局配置或默认10）
    $final_max_results = $max_results !== null ? $max_results : ($settings['max_results'] ?? 10);
