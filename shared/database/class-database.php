...
class Yali_AI_Writer_Database {
    ...
    public function create_tables() {
        ...
        $sql = 'CREATE TABLE IF NOT EXISTS `' . $api_configs_table . '` (
            ...
            `api_type` varchar(20) NOT NULL DEFAULT \'openai\' COMMENT \'大模型API类型: openai/gemini/claude\',
        ...
        $sql = 'CREATE TABLE IF NOT EXISTS `' . $publish_rules_table . '` (
            ...
            `structure_mode` varchar(20) NOT NULL DEFAULT \'generic\',
            ...
            `image_prompt_mode` varchar(50) NOT NULL DEFAULT \'default\' COMMENT \'配图提示词模式\',
            `image_prompt_template` text NOT NULL COMMENT \'图片提示词模板\',
            `enable_editor_assistant` tinyint(1) NOT NULL DEFAULT \'0\' COMMENT \'启用编辑器AI助手功能\',
            `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
            `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            PRIMARY KEY (`id`)
        ) ' . $charset_collate . ';';
        ...
        // 检查表是否创建成功
        $table_exists = $wpdb->get_var("SHOW TABLES LIKE '$gsc_used_keywords_table'") == $gsc_used_keywords_table;
        if (!$table_exists) {
            $errors[] = "Failed to create table: $gsc_used_keywords_table";
        } else {
            $created_tables[] = $gsc_used_keywords_table;
        }
    ...
    public function insert($table, $data) {
        ...
        if ($result === false) {
             error_log("ContentAuto Database Insert Error: " . $wpdb->last_error . " | Table: " . $table_name);
    ...
    public function update_api_configs_table_structure() {
        ...
        // 检查并添加 api_type 字段 (大模型API类型: openai/gemini/claude)
        $column_exists = $wpdb->get_var("SHOW COLUMNS FROM $table_name LIKE 'api_type'");
        if (!$column_exists) {
            $result = $wpdb->query("ALTER TABLE $table_name ADD COLUMN `api_type` varchar(20) NOT NULL DEFAULT 'openai' COMMENT '大模型API类型: openai/gemini/claude' AFTER `model_name`");
            if ($result !== false) {
                $updates_applied[] = "api_type字段";
            }
        }
  ...
  /**
    ...
    public function ensure_publish_rules_data_integrity() {
        ...
        $required_fields = [
            ...
            'image_prompt_mode' => [
                'sql' => "ALTER TABLE $table_name ADD COLUMN `image_prompt_mode` varchar(50) NOT NULL DEFAULT 'default' COMMENT '配图提示词模式' AFTER `role_description`",
                'description' => '图片提示词模式字段'
            ],
            'image_prompt_template' => [
                'sql' => "ALTER TABLE $table_name ADD COLUMN `image_prompt_template` text NOT NULL COMMENT '图片提示词模板' AFTER `image_prompt_mode`",
    ...
    private function populate_source_file_for_existing_templates($table_name) {
        ...
        $name_to_file_mapping = [
            ...
            'Topic Generation - GEO SEO Edition' => 'topic-geo-prompt.xml',
            'Article Generation - GEO SEO Edition' => 'article-geo-prompt.xml',
            // 新中文名称
            ...
            '主题生成 - GEO 搜索引擎优化版' => 'topic-geo-prompt.xml',
            '文章生成 - GEO 搜索引擎优化版' => 'article-geo-prompt.xml',
        ];
    ...
    private function seed_default_templates($table_name) {
        ...
        $templates_to_seed = [
            ...
            'article-geo-prompt.xml' => [
                'name' => '文章生成 - GEO 搜索引擎优化版',
                'type' => 'article_generation',
                'is_active' => false
            ],
            // 主题生成模板
            ...
            'topic-geo-prompt.xml' => [
                'name' => '主题生成 - GEO 搜索引擎优化版',
                'type' => 'topic_generation',
                'is_active' => false
            ]
        ];
        ...
        $is_non_chinese = strpos($locale, 'zh') !== 0;

        if ($is_non_chinese) {
            $en_names = [
                'article-generation-prompt-humanized-pro.xml' => 'Article Generation - Humanized Pro (V3.1)',
                'article-generation-prompt-optimized-v12.xml' => 'Article Generation - Optimized V12',
                'article-generation-prompt.xml' => 'Article Generation - Default',
                'article-generation-prompt1.xml' => 'Article Generation - Variant 1 (Detailed)',
                'article-generation-prompt2.xml' => 'Article Generation - Variant 2 (Concise)',
                'article-generation-prompt-technical.xml' => 'Article Generation - Technical Doc',
                'article-generation-prompt-academic.xml' => 'Article Generation - Academic Paper',
                'article-generation-prompt-casual.xml' => 'Article Generation - Casual Chat',
                'article-generation-prompt-xiaohongshu.xml' => 'Article Generation - Xiaohongshu',
                'article-generation-prompt-anti-ai.xml' => 'Article Generation - Anti-AI Detection',
                'article-generation-prompt-humanized.xml' => 'Article Generation - Humanized Polish',
                'article-generation-prompt-product-guide.xml' => 'Article Generation - Product Guide',
                'topic-generation-prompt.xml' => 'Topic Generation - Default',
                'topic1-generation-prompt.xml' => 'Topic Generation - Variant 1',
                'topic-generation-prompt-entity.xml' => 'Topic Generation - Entity',
                'topic-geo-prompt.xml' => 'Topic Generation - GEO SEO Edition',
                'article-geo-prompt.xml' => 'Article Generation - GEO SEO Edition',
            ];

            foreach ($templates_to_seed as $orig_file => &$template_info) {
                if (isset($en_names[$orig_file])) {
                    $template_info['name'] = $en_names[$orig_file];
                }
            }
            unset($template_info);
        }
        ...
        foreach ($templates_to_seed as $filename => $info) {
            ...
            // 非中文环境下自动去 en/ 目录读取同名文件
            if ($is_non_chinese) {
                $en_file_path = $templates_dir . 'en/' . $filename;
                if (file_exists($en_file_path)) {
