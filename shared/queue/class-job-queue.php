...
class Yali_AI_Writer_JobQueue {
        ...
        try {
            ...
            while ($processed_count < $max_jobs_per_run) {
                ...
                try {
                    ...
                    switch ($job['job_type']) {
                        case 'topic_task':
                            $result = $this->topic_task_manager->process_topic_task($job['job_id'], $job['subtask_id'], $job['id']);
                            break;
                        case 'article':
                            // 使用专门的文章队列处理器处理文章生成子任务
                            $result = $this->article_queue_processor->process_article_subtask($job['job_id'], $job['subtask_id'], $job['id']);
                            break;
                        case 'vector_generation':
                            // 给向量任务也注入 job_queue_id
                            $job['job_queue_id'] = $job['id'];
                            // 使用向量生成器处理向量生成任务
    ...
    private function update_article_parent_task_status($task_id) {
        ...
        if ($subtask_stats->failed == $subtask_stats->total) {
            // 所有子任务都失败
            $new_status = 'failed';
            $status_message = sprintf(__('All %d subtasks failed', 'yali-ai-writer'), $subtask_stats->total);
        } elseif ($subtask_stats->completed == $subtask_stats->total) {
            // 所有子任务都完成
            $new_status = 'completed';
            $status_message = sprintf(__('All %d subtasks completed', 'yali-ai-writer'), $subtask_stats->total);
            ...
            if ($subtask_stats->failed > 0) {
                ...
                if ($subtask_stats->completed > 0) {
                    $status_message = sprintf(__('%d subtask(s) completed, %d subtask(s) failed', 'yali-ai-writer'), $subtask_stats->completed, $subtask_stats->failed);
                } else {
                    $status_message = sprintf(__('All %d subtasks failed', 'yali-ai-writer'), $subtask_stats->failed);
                }
            } else {
                // 这种情况不应该发生，但为了安全起见
                $new_status = 'completed';
                $status_message = sprintf(__('All %d subtasks completed', 'yali-ai-writer'), $subtask_stats->completed);
            }
        } elseif ($subtask_stats->processing > 0) {
            // 有子任务正在处理中
            $new_status = 'processing';
            $status_message = sprintf(__('%d subtask(s) processing', 'yali-ai-writer'), $subtask_stats->processing);
        } elseif ($subtask_stats->pending > 0) {
            // 有子任务待处理
            $new_status = 'pending';
            $status_message = sprintf(__('%d subtask(s) pending', 'yali-ai-writer'), $subtask_stats->pending);
        }
