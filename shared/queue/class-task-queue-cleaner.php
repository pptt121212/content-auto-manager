...
class Yali_AI_Writer_TaskQueueCleaner {
     ...
     *  - clean_topic_tasks: bool (default false) 清理已完成的主题任务
     *  - clean_article_tasks: bool (default false) 清理已完成的文章任务
     * 
     * @return array 清理统计
     */
    public function clean_completed_data($options = array()) {
        global $wpdb;
        
        $stats = array(
            'topic_tasks_deleted' => 0,
            'article_tasks_deleted' => 0
        );

        $defaults = array(
            'clean_topic_tasks' => false,
            'clean_article_tasks' => false
        );
        ...
        if ($options['clean_topic_tasks']) {
            ...
            $this->log('BULK_CLEAN', '批量清理已完成主题任务', ['count' => $stats['topic_tasks_deleted']]);
        }
        ...
        if ($options['clean_article_tasks']) {
            ...
            $this->log('BULK_CLEAN', '批量清理已完成文章任务', ['count' => $stats['article_tasks_deleted']]);
        }
