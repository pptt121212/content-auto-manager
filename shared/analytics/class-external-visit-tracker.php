...
class ContentAuto_ExternalVisitTracker {
     ...
     * 获取文章的外部访问统计（简化版）
     */
    public function ajax_get_visit_stats() {
        check_ajax_referer('external_visit_stats_nonce', 'nonce');
        
        if (!current_user_can('manage_options')) {
            wp_send_json_error(array('message' => __('您没有权限查看此统计', 'yali-ai-writer')));
            return;
        }
        ...
        $stats = $this->get_external_visit_stats($post_id);
