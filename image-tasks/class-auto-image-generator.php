...
class Yali_AI_Writer_AutoImageGenerator {
    ...
    public function auto_generate_images_for_post($post_id, $content, $options = []) {
        ...
        $this->log_debug('AUTO_IMAGE_START', '开始为文章自动生成图片', $context);
        ...
        try {
            ...
            // 【调试】详细记录配置读取结果
            $this->log_debug('CONFIG_DEBUG', '发布规则配置读取结果', array_merge($context, [
            ...
            if ($skip_first && $total_found > 0) {
                ...
                $this->log_debug('FIRST_PLACEHOLDER_SKIPPED', '跳过首个图片占位符', array_merge($context, [
                    'total_found' => $total_found,
            ...
            if (count($placeholders) > $max_images) {
                ...
                $this->log_debug('PLACEHOLDERS_LIMITED', '图片占位符数量受限', array_merge($context, [
                    'total_found' => $total_found,
            ...
            $this->log_debug('PLACEHOLDERS_FOUND', '找到图片占位符', array_merge($context, [
            ...
            // 【关键修复】反向处理占位符（从后往前），确保偏移量在替换过程中保持有效
            // 这样前面的替换不会影响后面占位符的偏移量
            $placeholders_to_process = array_reverse($placeholders);
            
            foreach ($placeholders_to_process as $placeholder) {
                // 仅在调试模式下记录处理占位符的详细信息
                $this->log_debug('PROCESSING_PLACEHOLDER', '处理图片占位符', array_merge($context, ['prompt' => $placeholder['prompt']]));
    ...
    private function generate_and_replace_placeholder($placeholder, &$content, $post_id) {
        ...
        try {
            ...
            // 4. 替换占位符
            // 【关键修复】使用基于偏移量的精确替换，而不是全文本查找替换
            // 这能避免因特殊字符、编码问题或重复内容导致的替换失败
            $offset = $placeholder['offset'];
            
            // 【修复】使用多字节安全的函数处理内容
            // 注意：preg_match 返回的 offset 是字节偏移量，不是字符偏移量
            $length = strlen($placeholder['full_match']); // 占位符是ASCII，strlen安全
            
            // 验证偏移量处的内容是否匹配（双重保险）
            // 使用 substr 因为是字节偏移量
            if (substr($content, $offset, $length) === $placeholder['full_match']) {
                // 使用 substr_replace 进行字节级别的替换
                $content = substr_replace($content, $image_html, $offset, $length);
                $this->log_debug('REPLACEMENT_BY_OFFSET', '使用偏移量替换占位符成功', array_merge($context, [
                    'offset' => $offset,
                    'length' => $length
                ]));
            } else {
                // 如果偏移量内容不匹配（极少情况），回退到全文本替换
                // 【修复】使用多字节安全的 str_replace
                $this->logger->log_warning('OFFSET_MISMATCH', '偏移量内容不匹配，回退到全文本替换', array_merge($context, [
                    'offset' => $offset,
                    'expected_start' => substr($placeholder['full_match'], 0, 20),
                    'actual_start' => substr($content, $offset, 20)
                ]));
     ...
     * @param bool $skip_save 是否跳过保存到媒体库（默认false）。如果为true，将返回Base64 Data URI格式的图片。
     * @return string|false 图片HTML或失败时返回false
     */
    public function generate_single_image($prompt, $post_id = 0, $skip_save = false) {
        try {
            // 检查图像API是否配置
            if (!$this->is_image_api_configured()) {
            ...
            // --- 新增：如果跳过保存，直接返回 Data URI ---
            if ($skip_save) {
                // 检测图片类型以构建正确的 Data URI
                ...
                // 返回带有 Data URI 的 HTML
                // 注意：这里没有 attachment_id，也没有 post_id 关联
                $image_html = sprintf(
                ...
                $this->logger->log_success('SINGLE_IMAGE_TRANSIENT', '单个图片生成成功（不保存）', [
                    'prompt' => $prompt,
                    'post_id' => $post_id
                ]);
    ...
    public function schedule_image_generation($post_id, $content, $publish_rules = null) {
        ...
        // 【关键修复】动态注册这个特定任务的处理器
        // 使用闭包直接处理，避免依赖异步处理器类
        $logger = $this->logger; // 捕获logger供闭包使用
        add_action($hook_name, function($task_post_id, $task_content, $task_publish_rules = null) use ($logger) {
            $context = [
                'post_id' => $task_post_id,
                'function' => 'dynamic_process_post_images',
                'timestamp' => current_time('mysql'),
                'has_publish_rules' => !empty($task_publish_rules)
            ];
            
            $logger->log_info('ASYNC_IMAGE_START', '开始异步处理文章图片（动态处理器）', $context);
            
            try {
                // 检查文章是否仍然存在
                $post = get_post($task_post_id);
                if (!$post) {
                    $logger->log_error('POST_NOT_FOUND', '文章不存在，跳过图片处理', $context);
                    return;
                }
                
                // 检查是否已处理过，避免重复处理
                $already_processed = get_post_meta($task_post_id, '_auto_images_processed', true);
                if ($already_processed === '1') {
                    $logger->log_info('ALREADY_PROCESSED', '文章已处理过图片，跳过', $context);
                    return;
                }
                
                // 获取最新的文章内容（可能已被更新）
                $current_content = $post->post_content;
                if ($task_content !== $current_content) {
                    $logger->log_info('CONTENT_UPDATED', '检测到文章内容已更新，使用最新内容', $context);
                ...
                $result = $image_generator->auto_generate_images_for_post($task_post_id, $task_content, $options);
                
                if ($result['success']) {
                    $logger->log_success('ASYNC_IMAGE_SUCCESS', '异步图片处理完成', array_merge($context, [
                        'generated_count' => $result['generated_count'],
                        'failed_count' => $result['failed_count'] ?? 0
                    ]));
                    
                    // 更新文章记录，标记已处理图片
                    update_post_meta($task_post_id, '_auto_images_processed', true);
                    update_post_meta($task_post_id, '_auto_images_count', $result['generated_count']);
                    update_post_meta($task_post_id, '_auto_images_processed_time', current_time('mysql'));
                } else {
                    $logger->log_error('ASYNC_IMAGE_FAILED', '异步图片处理失败', array_merge($context, [
                        'error' => $result['error'] ?? '未知错误'
                    ]));
                }
                
            } catch (Exception $e) {
                $logger->log_error('ASYNC_IMAGE_EXCEPTION', '异步图片处理异常', array_merge($context, [
                    'exception_message' => $e->getMessage()
                ]));
            }
        }, 10, 3);
        
        // 调度一个异步任务，加入极短延迟（2秒）以确保当前请求完成，但尽快执行避免遗漏
        // 【修复】使用随机延迟 2-7 秒，进一步错开多个任务的执行时间
        $delay = 2 + mt_rand(0, 5);
        wp_schedule_single_event(time() + $delay, $hook_name, $task_data);
        
        $this->logger->log_info('IMAGE_GENERATION_SCHEDULED', '图片生成任务已调度（独立hook+动态处理器）', [
            'post_id' => $post_id,
            'hook_name' => $hook_name,
