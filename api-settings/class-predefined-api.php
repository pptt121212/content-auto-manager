...
class Yali_AI_Writer_PredefinedApi {
    ...
    public function get_channels() {
        ...
        return array(
            'pollinations' => array(
                'name' => __('pollinations渠道', 'yali-ai-writer'),
                'api_url' => 'https://gen.pollinations.ai/'
            ),
            'official' => array(
                'name' => __('插件官方API', 'yali-ai-writer'),
                'api_url' => 'https://key.kdjingpai.com/api-proxy.php'
            )
        );
    }
    ...
    public function create_config_record($channel = 'pollinations', $is_active = 0) {
        ...
        $data = array(
            ...
            'model_name' => ($channel === 'official') ? __('官方API', 'yali-ai-writer') : 'openai-large', 
    ...
    public function update_config($channel, $data) {
        ...
        // 只允许更新特定字段
        $allowed_fields = array('is_active', 'api_key', 'model_name');
        $update_data = array();
    ...
    public function validate_config_readiness($channel = 'pollinations') {
        ...
        try {
            ...
            if (!isset($this->channels[$channel])) {
                $result['errors'][] = __('渠道不存在: ', 'yali-ai-writer') . $channel;
                return $result;
            }
            ...
            $config = $this->get_config($channel);
            if (!$config) {
                $result['errors'][] = __('配置不存在: ', 'yali-ai-writer') . $channel;
                return $result;
            }
            ...
            if ($config['is_active'] != 1) {
                $result['errors'][] = __('配置未激活: ', 'yali-ai-writer') . $channel;
                return $result;
            }
            ...
            foreach ($required_fields as $field) {
                if (empty($config[$field])) {
                    $result['errors'][] = __('配置缺少必要字段 ', 'yali-ai-writer') . $field . ': ' . $channel;
                    return $result;
                }
            }
            ...
            $result['errors'][] = __('配置验证异常: ', 'yali-ai-writer') . $e->getMessage();
        }
    ...
    public function send_request($channel, $prompt) {
        ...
        $channel_obj = $this->get_channel($channel);
        if (!$channel_obj) {
            return array('success' => false, 'message' => __('无效的API渠道', 'yali-ai-writer'));
        }
        
        // 获取数据库配置
        $config = $this->get_config($channel);
        if (!$config) {
            return array('success' => false, 'message' => __('未找到API配置', 'yali-ai-writer'));
        }
    ...
    public function test_connection($channel = 'pollinations') {
        ...
        if (!$config) {
            ...
            $config = array(
                ...
                'api_url' => $channel_obj->get_api_url(),
                'api_key' => '',
                'model_name' => __('预置模型', 'yali-ai-writer'),
                'temperature' => 0.7,
                'max_tokens' => 2000,
