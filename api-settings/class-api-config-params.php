...
class ContentAuto_ApiConfigParams {
    ...
    public $temperature_enabled = 0;
    public $max_tokens_enabled = 0;
    public $is_active = 0;
    ...
    public function validate() {
        ...
        // 验证必需字段
        if (empty($this->name)) {
            $errors[] = __('配置名称不能为空', 'yali-ai-writer');
        }

        // 如果不是预置API，验证API地址和模型名称
        if (empty($this->predefined_channel)) {
            if (empty($this->api_url)) {
                $errors[] = __('API地址不能为空', 'yali-ai-writer');
            }

            if (empty($this->model_name)) {
                $errors[] = __('模型名称不能为空', 'yali-ai-writer');
            }
        }

        // 验证数值范围
        if ($this->temperature < 0 || $this->temperature > 2) {
            $errors[] = __('温度参数必须在0-2之间', 'yali-ai-writer');
        }

        if ($this->max_tokens < 1 || $this->max_tokens > 32000) {
            $errors[] = __('最大Token数必须在1-32000之间', 'yali-ai-writer');
        }

        // 验证新增参数范围
        if ($this->top_p < 0 || $this->top_p > 1) {
            $errors[] = __('top_p参数必须在0-1之间', 'yali-ai-writer');
        }
