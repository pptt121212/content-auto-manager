...
class ContentAuto_OfficialChannel {
    ...
    public function get_name() {
        return __('插件官方API', 'yali-ai-writer');
    }
    ...
    public function get_description() {
        return __('使用插件官方提供的API服务，通过授权码验证使用。如需获取授权码，请联系插件作者微信：qn006699 获取插件授权码后使用。', 'yali-ai-writer');
    }
    ...
    public function send_request($config, $prompt) {
        ...
        if (empty($license_key)) {
            return array('success' => false, 'message' => __('未配置授权码', 'yali-ai-writer'));
        }
    ...
    public function debug_license_info() {
        ...
        $debug_info['license_message'] = isset($license_data['message']) ? $license_data['message'] : __('无消息', 'yali-ai-writer');
    ...
    public function get_quota_info() {
        ...
        if (empty($license_key)) {
            return array(
                'success' => false, 
                'message' => __('未配置授权码，请在发布规则中设置授权码', 'yali-ai-writer')
            );
        }
        ...
        if (empty($domain)) {
            return array(
                'success' => false, 
                'message' => __('无法获取当前域名', 'yali-ai-writer')
            );
        }
        ...
        if (is_wp_error($response)) {
            return array(
                'success' => false, 
                'message' => __('请求失败: ', 'yali-ai-writer') . $response->get_error_message()
            );
        }
        ...
        if ($response_code !== 200) {
            return array(
                'success' => false, 
                'message' => __('服务器响应错误: HTTP ', 'yali-ai-writer') . $response_code
            );
        }
        ...
        if (!$data) {
            return array(
                'success' => false, 
                'message' => __('响应数据格式错误', 'yali-ai-writer')
            );
        }
        ...
        if ($data['status'] === 'success') {
            return array(
                ...
                'message' => __('配额信息获取成功', 'yali-ai-writer')
            );
        } else {
            return array(
                'success' => false, 
                'message' => isset($data['message']) ? $data['message'] : __('获取配额信息失败', 'yali-ai-writer')
            );
        }
    }
    ...
    public function test_connection($config) {
        ...
        if (empty($license_key)) {
            ...
            $debug_message = __('未配置授权码，请在发布规则中设置授权码。', 'yali-ai-writer');
            $debug_message .= __(' 调试信息：', 'yali-ai-writer');
            $debug_message .= __(' 存储位置=', 'yali-ai-writer') . $debug_info['storage_location'];
            $debug_message .= __(', 选项键=', 'yali-ai-writer') . $debug_info['option_key'];
            $debug_message .= __(', 授权码=', 'yali-ai-writer') . ($debug_info['license_key'] ? $debug_info['license_key'] : '(空)');
            $debug_message .= __(', 长度=', 'yali-ai-writer') . $debug_info['license_key_length'];
            $debug_message .= __(', 状态=', 'yali-ai-writer') . $debug_info['license_status'];
        ...
        if (is_wp_error($response)) {
            return array(
                'success' => false, 
                'message' => __('连接失败: ', 'yali-ai-writer') . $response->get_error_message()
            );
        }
        ...
        if ($data['status'] === 'success') {
            return array(
                'success' => true, 
                'message' => __('连接成功', 'yali-ai-writer')
            );
        } else {
            return array(
                'success' => false, 
                'message' => isset($data['message']) ? $data['message'] : __('连接失败', 'yali-ai-writer')
            );
        }
    }
